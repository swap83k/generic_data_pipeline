...
import pandas as pd, json as js, argparse, os, sys,pathlib
...
def generate_ctl(dataset):
	...
	header.to_csv(os.path.join(ROOT_DIR,'sink',f'control_{file_date}.dat'), index=False, sep=',')
	print(f'control_{file_date}.dat' + ' generated..')	
	return file_date
...
def dq_apply(as_on_date,dataset):
    ...
    filtered_data.to_csv(os.path.join(ROOT_DIR,'sink',f'filtered_{as_on_date}.dat'), index=False, sep=',')
    print(f'filtered_{as_on_date}.dat' + ' generated..')
    ...
    clean_data.to_csv(os.path.join(ROOT_DIR,'sink',f'gdp_{as_on_date}.dat'), index=False, sep=',')
    print(f'gdp_{as_on_date}.dat' + ' generated..')

def ingest_data(gdp_data,filtered_data,control_data):
	print('Calling data ingest')
